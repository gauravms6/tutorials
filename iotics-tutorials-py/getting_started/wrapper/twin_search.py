...
from helpers.utilities import make_api_call, print_property_grpc,extract_twin_locations_to_csv,save_twin_properties_to_csv,save_twins_feeds_inputs,Dsave_twin_properties_to_csv
...
def main():
    ...
    for response in iotics_api.search_iter(
        client_app_id="search_twins", payload=search_criteria,scope = 'GLOBAL'
    ):
    ...
    for twin in twins_found_list:
        extract_twin_locations_to_csv(twins_found_list, 'twins_locations.csv')
        save_twin_properties_to_csv(twins_found_list, 'twin_properties.csv')
        save_twins_feeds_inputs(twins_found_list, 'twins_feeds_inputs.csv')
