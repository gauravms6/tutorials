...
def save_twinKeyName_to_csv(data, csv_file):
    if not os.path.exists(csv_file):
        ...
        # Check if CSV file already contains the data
        if pd.Series(data).equals(pd.read_csv(csv_file).iloc[-1]):
            print("Data already exists in", csv_file)
        else:
            # Create DataFrame from dictionary
            df = pd.DataFrame(data, index=[0])
            ...
            if existing_df.isin(data).all(axis=1).any():
                # Replace the existing row with the new data
                existing_df.update(df)
                existing_df.to_csv(csv_file, index=False)
                print("Data replaced in", csv_file)
            else:
                # Append new data to existing DataFrame
                df.to_csv(csv_file, mode='a', header=False, index=False)
                print("Data appended to", csv_file)

def get_did_from_csv(twin_key_name, csv_file):
    # Read CSV file into DataFrame
    df = pd.read_csv(csv_file)
    
    # Search for the row where 'TWIN_KEY_NAME' matches the given input
    row = df[df['TWIN_KEY_NAME'] == twin_key_name]
    
    # Check if any rows were found
    if len(row) == 0:
        return None
    else:
        # Retrieve the DID from the matching row
        twin_did = row.iloc[0]['TWIN_DID']
        return twin_did